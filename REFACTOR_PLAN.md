...
**DevOps learning doc**: alongside building the above, write `DEVOPS.md` (root) explaining how and why this repo's CI/CD, Dependabot, and pre-commit setup work — for the project owner, who has some DevOps exposure (seen CI/Docker on other projects) but hasn't configured it themselves. Skip 101-level "what is CI" explanations; go straight to this repo's actual `ci.yml`/`dependabot.yml`/husky config, what triggers each job, how to read a failed run, and why each check exists. Same spirit as `server/LEARNING.md` — teach from the real files just written, not generic tutorials.
